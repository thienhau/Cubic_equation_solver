...
module fp_fma #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            s2_valid <= 0; s2_mant_mul <= 0; s2_mant_c_aligned <= 0;
        end else begin
            s2_valid <= s1_valid;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s3_wide_sum == 0) begin
                ...
                // FIX: Điều chỉnh lại exp và dịch >> 25 để hạ MSB từ bit 72 xuống bit 47 an toàn, không bị văng bit
                s4_exp_res <= s3_exp_max - lza_shift + 9'd26; 
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            out_valid <= s4_valid;
            if (s4_valid) begin
                if (s4_mant_norm == 0 || s4_exp_res[8]) begin
                    out_result <= 32'd0; 
                end else begin
