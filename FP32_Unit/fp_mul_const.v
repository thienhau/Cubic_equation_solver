...
module fp_mul_const_one_third #(
    ...
    // Mở rộng thành 56-bit để tránh mất bit khi dịch phải chuỗi hình học
    wire [55:0] in_mant_ext = {in_mant, 32'd0}; 

    // T = 0 -> 1: Thực hiện dịch bit song song mô phỏng phép nhân với 0.33333333
    reg        s1_valid;
    reg        s1_sign;
    reg [7:0]  s1_exp;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (in_valid && in_exp != 0) begin
                s1_sign <= in_sign;
                s1_exp  <= in_exp;
                // Gom các nhóm dịch bit của chuỗi tổng vô hạn 1/4 + 1/16 + 1/64...
    ...
    // T = 1 -> 2: Tổng hợp kết quả dịch, chuẩn hóa mantissa và làm tròn GRS
    wire [55:0] sum_total = s1_sum_A + s1_sum_B + s1_sum_C + s1_sum_D;

    reg [23:0]       norm_mant;
    reg signed [8:0] norm_exp; 
    reg              G, R, S, LSB;

    always @(*) begin
        // Quét bit nguyên ngầm định sau phép nhân 1/3 để quyết định dải dịch trái chuẩn hóa
    ...
    // Thực hiện làm tròn theo phương thức RNE
