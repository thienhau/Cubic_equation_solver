...
module fp_div #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            s1_valid <= in_valid;
            if (in_valid) begin
                // Báo lỗi bất định: Chia cho 0 (mẫu bằng 0) HOẶC phép toán 0/0
    ...
    // Giữ nguyên thiết kế mảng từ 3 đến 8 như code cũ trong repo
    reg        s_valid [3:8];
    reg        s_sign  [3:8];
    reg [8:0]  s_exp   [3:8];
    reg        s_zero  [3:8];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (j = 0; j < 4; j = j + 1) begin // Vòng lặp cũ lặp 4 bit
                t_rem = t_rem << 1;
            ...
            for (i = 4; i <= 8; i = i + 1) begin
                ...
                s_rem[i] <= t_rem[24:0];
                s_q[i]   <= {s_q[i-1][21:0], t_q};
            end
        end
    end

    // Giữ nguyên các tầng delay p_valid từ 9 đến 13 giống code cũ repo
    reg        p_valid [9:13];
    reg        p_sign  [9:13];
    reg [8:0]  p_exp   [9:13];
    reg        p_zero  [9:13];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (p_valid[13]) begin
                if (p_inv[13]) begin
                    out_result     <= {p_sign[13], 8'hFF, 23'h3FFFFF}; // Trả về Quiet NaN chuẩn
