...
module cardano_top #(
    ...
    // T = 0 -> 0: Phân loại hệ số nhánh đầu vào
    ...
    fp_div u_div_a (
        .clk(clk), .rst_n(rst_n), .in_valid(valid_in), 
        .in_operand_A(32'h3F800000), .in_operand_B(a), 
        .out_valid(v14), .out_result(inv_a),
    ...
    // T = 14 -> 18: A = b/a, B = c/a, C = d/a
    wire [31:0] A_coef, B_coef, C_coef; wire v18;
    fp_mul u_mul_A (
        .clk(clk), .rst_n(rst_n), .in_valid(v14), 
        .in_operand_A(b14), .in_operand_B(inv_a), 
        .out_valid(v18), .out_result(A_coef),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    
    fp_mul u_mul_B (
        .clk(clk), .rst_n(rst_n), .in_valid(v14), 
        .in_operand_A(c14), .in_operand_B(inv_a), 
        .out_valid(), .out_result(B_coef),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    
    fp_mul u_mul_C (
        .clk(clk), .rst_n(rst_n), .in_valid(v14), 
        .in_operand_A(d14), .in_operand_B(inv_a), 
        .out_valid(), .out_result(C_coef),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );

    // T = 18 -> 20: S = A / 3
    wire [31:0] S_val; wire v20;
    fp_mul_const_one_third u_mul_S (
        .clk(clk), .rst_n(rst_n), .in_valid(v18), 
        .in_operand_A(A_coef), 
        .out_valid(v20), .out_result(S_val)
    );
    ...
    // T = 20 -> 24: S2 = S*S, SB = S*B
    wire [31:0] S2, SB; wire v24;
    fp_mul u_mul_S2 (
        .clk(clk), .rst_n(rst_n), .in_valid(v20), 
        .in_operand_A(S_val), .in_operand_B(S_val), 
        .out_valid(v24), .out_result(S2),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    
    fp_mul u_mul_SB (
        .clk(clk), .rst_n(rst_n), .in_valid(v20), 
        .in_operand_A(S_val), .in_operand_B(B20), 
        .out_valid(), .out_result(SB),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    ...
    fp_mul u_mul_S3 (
        .clk(clk), .rst_n(rst_n), .in_valid(v24), 
        .in_operand_A(S2), .in_operand_B(S24), 
        .out_valid(v28), .out_result(S3),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    
    fp_mul u_mul_3S2 (
        .clk(clk), .rst_n(rst_n), .in_valid(v24), 
        .in_operand_A(S2), .in_operand_B(32'h40400000), 
        .out_valid(), .out_result(mul_3S2),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    
    fp_add_sub u_sub_C_SB (
        .clk(clk), .rst_n(rst_n), .in_valid(v24), .in_is_sub(1'b1), 
        .in_operand_A(C24), .in_operand_B(SB), 
        .out_valid(), .out_result(C_minus_SB),
    ...
    fp_add_sub u_sub_p (
        .clk(clk), .rst_n(rst_n), .in_valid(v28), .in_is_sub(1'b1), 
        .in_operand_A(B28), .in_operand_B(mul_3S2), 
        .out_valid(v32), .out_result(p_val_int),
        .status_overflow(), .status_zero()
    );
    
    fp_mul u_mul_2S3 (
        .clk(clk), .rst_n(rst_n), .in_valid(v28), 
        .in_operand_A(S3), .in_operand_B(32'h40000000), 
        .out_valid(), .out_result(mul_2S3),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    ...
    fp_add_sub u_add_q (
        .clk(clk), .rst_n(rst_n), .in_valid(v32), .in_is_sub(1'b0), 
        .in_operand_A(mul_2S3), .in_operand_B(C_minus_SB32), 
        .out_valid(v36), .out_result(q_val_int),
        .status_overflow(), .status_zero()
    );
    
    // T = 32 -> 34: p/3
    wire [31:0] p_3; wire v34;
    fp_mul_const_one_third u_mul_p3 (
        .clk(clk), .rst_n(rst_n), .in_valid(v32), 
        .in_operand_A(p_val_int), 
        .out_valid(v34), .out_result(p_3)
    );
    ...
    fp_mul u_mul_q2 (
        .clk(clk), .rst_n(rst_n), .in_valid(v36), 
        .in_operand_A(q_val_int), .in_operand_B(32'h3F000000), 
        .out_valid(v40), .out_result(q_2),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    ...
    // T = 40 -> 44: (q/2)^2 và (p/3)^2
    wire [31:0] q_2_sq, p_3_sq; wire v44;
    fp_mul u_mul_q2_sq (
        .clk(clk), .rst_n(rst_n), .in_valid(v40), 
        .in_operand_A(q_2), .in_operand_B(q_2), 
        .out_valid(v44), .out_result(q_2_sq),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    
    fp_mul u_mul_p3_sq (
        .clk(clk), .rst_n(rst_n), .in_valid(v40), 
        .in_operand_A(p_3_40), .in_operand_B(p_3_40), 
        .out_valid(), .out_result(p_3_sq),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    ...
    fp_mul u_mul_p3_cb (
        .clk(clk), .rst_n(rst_n), .in_valid(v44), 
        .in_operand_A(p_3_sq), .in_operand_B(p_3_44), 
        .out_valid(v48), .out_result(p_3_cb),
        .status_overflow(), .status_underflow(), .status_invalid(), .status_zero()
    );
    ...
    // T = 48 -> 52: Delta = (q/2)^2 + (p/3)^3
    wire [31:0] delta_val_int; wire v52_delta;
    fp_add_sub u_add_delta (
        .clk(clk), .rst_n(rst_n), .in_valid(v48), .in_is_sub(1'b0), 
        .in_operand_A(q_2_sq_48), .in_operand_B(p_3_cb), 
        .out_valid(v52_delta), .out_result(delta_val_int),
        .status_overflow(), .status_zero()
    );
    ...
    // T = 52 -> *: FIFO Metadata bypass cho định tuyến nghiệm
